...
/// <summary>
/// If the input list has only one element, returns that element.
/// If the input list has more or less than one element, returns None.  
/// </summary>
...
/// If the input function is f and the elements are i0...iN and j0...jN
/// then computes f (... (f s i0 j0)...) iN jN.
/// Returns None if the input lists have a different number of elements.
/// </summary>
...
/// <c>List.forall2</c> will throw.
/// </summary>
let tryForall2 predicate xs ys = forall2Safe predicate xs ys |> Result.toOption
...
/// Returns None if <c>xs</c> has no elements.
/// </summary>
let inline tryMax<'a when 'a : comparison> (xs:'a list) = maxSafe xs |> Result.toOption
...
/// Returns None if <c>xs</c> has no elements.
/// </summary>
let inline tryMaxBy<'a, 'b when 'b : comparison> (projection:'a -> 'b) (xs:'a list) = maxBySafe projection xs |> Result.toOption
...
/// Returns None if <c>xs</c> has no elements.
/// </summary>
let inline tryMin<'a when 'a : comparison> (xs:'a list) = minSafe xs |> Result.toOption
...
/// Returns None if <c>xs</c> has no elements.
/// </summary>
let inline tryMinBy<'a, 'b when 'b : comparison> (projection:'a -> 'b) (xs:'a list) = minBySafe projection xs |> Result.toOption
...
/// <summary>
/// Returns the transpose of the given sequence of lists.  Returns None if
/// the input lists differ in length. 
/// </summary>
...
module NonEmpty =
  ...
  /// </summary>
  let tryMap2 f xs ys : Option<NonEmptyList<_>> = map2' f xs ys |> Result.toOption
